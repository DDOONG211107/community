...
router.post("/:notice_idx", checkIsLogged, async (req, res) => {
  ...
  try {
    ...
    if (data.rows.length == 1) {
      ...
      if (data2.rows.length == 0) {
        ...
        client.end();
        result.message = "server: 좋아요 삽입 실패 -> 롤백했음";
...
router.delete("/:notice_idx", checkIsLogged, async (req, res) => {
  const { accountIdx } = req.session;
  const { notice_idx } = req.params;
  ...
  try {
    ...
    if (data.rows.length == 0) {
      ...
      if (data2.rows.length == 0) {
        ...
        result.message = "server: 좋아요 취소 실패";
        res.status(500).send(result);
        ...
        if (data3.rows.length == 0) {
          ...
          await client.query(`COMMIT`);
          client.end();
          result.message = "server: rollback => like failed";
          res.status(500).send(result);
